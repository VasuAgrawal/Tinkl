...
public class HistoryDisplayActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            for(byte[] b : samples) {
                history.add(Sample.parseFrom(b));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for (int i = 0; i < history.size(); i++) {
            temperatureEntries.add(new Entry(i, history.get(i).getTemperature()));
            turbidityEntries.add(new Entry(i, history.get(i).getTurbidity()));
            colorEntries.add(new BarEntry(i, 1));
            float r = history.get(i).getColor().getR();
            float g = history.get(i).getColor().getG();
            float b = history.get(i).getColor().getB();
